...
- [Part II: Deciding What Deserves Organizational Attention](parts/part-2.md)
- [The Front Door](chapters/02-front-door.md)
- [Future Edition: Portfolio Judgment](chapters/03-portfolio-judgment.md)
- [Part III: Framing and Authorizing Work](parts/part-3.md)
- [Frame the Work Before Designing It](chapters/04-frame-before-design.md)
- [Make the Commitment Legitimate](chapters/05-legitimate-commitment.md)
...
- [Development Is an Organizational Architecture](chapters/11-development-architecture.md)
- [Management Is Evidence Infrastructure](chapters/12-management-evidence.md)
- [Create Safe Reps and Allocate Opportunity](chapters/13-safe-reps-and-opportunity.md)
- [Future Edition: Hiring Capability](chapters/14-hiring-capability.md)
- [Selecting External Capability](chapters/15-selecting-external-capability.md)
- [Proving and Accepting External Capability](chapters/16-proving-external-capability.md)
- [Part VI: Reconciling the Enterprise](parts/part-6.md)
- [Future Edition: Reconciling the Enterprise](chapters/17-reconciling-enterprise.md)
- [Future Edition: AI, Context, and Organizational Leverage](chapters/18-ai-context-leverage.md)
- [An Implementation Roadmap](chapters/19-implementation-roadmap.md)

- [Reusable Instruments and References](appendices/00-appendices.md)
- [Appendix A: Framing and Work-Item Tools](appendices/a-framing-and-work-item-tools.md)
- [Appendix B: Conversation Planner](appendices/b-conversation-planner.md)
- [Appendix C: Talent Development Workbook](appendices/c-tda-workbook.md)
...
## The decision lifecycle {.unnumbered .unlisted}
...
## Claims, records, and evidence {.unnumbered .unlisted}
...
## Work and completion {.unnumbered .unlisted}
...
<!-- Preview assembly source: Book assembly synthesis from repeated terminology across the current source corpus -->


\newpage

# Part I: Learning to See the Loop {.part .unnumbered}
...
# Part II: Deciding What Deserves Organizational Attention {.part .unnumbered}
...
# Future Edition: Portfolio Judgment {.unnumbered}
...
> **Preview note — future edition**
>
> A future version will define the comparative decision: who ranks eligible proposals, which evidence must be comparable, how funding and named capacity enter the decision, how regulatory or urgent work changes the queue, what an approval displaces, and when a forecast change requires reprioritization.
>
...
## The minimum shape {.unnumbered .unlisted}
...
Once the organization has selected a claim on its attention, the work is still not ready for implementation. Selection authorizes deeper judgment; it does not make the design true.

<!-- Preview assembly source: 0.Organizational-Excellence-Book-Gaps.md synthesis; mechanisms from Work Intake, Process Improvement, and Writing Work Items -->


\newpage

# Part III: Framing and Authorizing Work {.part .unnumbered}
...
An **output** is an activity or artifact: *"build a base image,"* *"submit a PR."* An output is "done" when the activity stops - which is to say, never cleanly, because there's always more you *could* do.

An **outcome** is a change in the world, stated as a condition that is either true or false: *"all hosts provision from one image with zero manual steps,"* *"instances auto-register to the correct regional endpoint on first boot."* An outcome is done when the condition becomes true. You can *check* it.
...
Everything else in this chapter exists to make that rule operable.
...
- image-based configs tracked in the image-build repo
- environment-specific configs do not run in environments where they do not apply
- startup configuration scripts modular and managed in code
- scripts have logging
- every config has a regression/validation test in a post-build step

Read that list again. **Not one item tells you when the *epic* is finished** - at best they're per-*child-task* quality bars. "Enabling and building a base image" is an activity with no end state - so it ends when we get tired of it, which is exactly the problem we're trying to kill.

(And only *at best*: two of these aren't even clean quality bars. "Environment-specific conditionals" misunderstands the epic - a *golden/base* image holds only what **all** systems need, so per-environment conditionals don't belong in it at all; environment divergence lives in image *layering* or runtime metadata, not baked-in `if-cloud` logic. "Modular... in code" dictates an *architecture*, which a Definition of Done has no business mandating. We dissect why in [the distinction among DoD, acceptance, and outcome](#three-things-people-confuse-dod-vs-acceptance-vs-outcome); for now just note that a sloppy DoD is hiding an *absent* outcome.)

What's missing is an **outcome / exit condition** for the epic itself. Something like:

> *A single base image provisions Linux hosts across two runtime types with zero manual post-build steps; >=20 consecutive automated builds pass post-build validation; the legacy build path is decommissioned.*

That sentence is checkable. Today's epic is not. We'll fix it properly in [the distinction among DoD, acceptance, and outcome](#three-things-people-confuse-dod-vs-acceptance-vs-outcome).
...
> **Who owns what (this matters for the rest of the chapter):** an epic is, in effect, a **broad-stroke request from the business** - and its **Outcome** and **Definition of Done are owned by the requester / standard-owner, not by the implementers.** The people choosing *how* to build it don't get to rewrite the bar they're measured against. The practical consequence shows up in [the distinction among DoD, acceptance, and outcome](#three-things-people-confuse-dod-vs-acceptance-vs-outcome): because the DoD isn't the implementers' to write, it can only hold things the business can legitimately demand - **substitutable, checkable invariants** ("works on both runtimes," "complies with our standards") - never *architecture choices* like "make it modular," which would be the requester reaching into the implementers' domain.
...
1. **A Discovery / decision** - *"This work item will identify a method for dynamically targeting the correct regional registration endpoint."* Done when a decision is recorded.
2. **An implementation** - the startup configuration script that uses metadata to fetch the regional secret. Done when it is merged and works.
3. **Infrastructure changes** - *"create the secret,"* *"grant the instance role access to the secret."* Done when those resources exist and grant access.
...
![The problem-version task separates into a Discovery work package whose decision feeds a behavior-focused Story, with concrete infrastructure changes as child Tasks.](assets/images/writing-work-items/split-by-completion-condition.svg){#fig-split-by-completion-condition}

---

## Writing outcomes (for epics)

> **Scope:** outcomes are a **roadmap-work** construct. Reactive demand items - incidents, standard requests - are *not* given outcome statements; they're governed by resolution and structured capture. This section is about the improvement epics we *choose* to run.
...
> **The substitutability test - outcome vs. implementation.** The trap isn't vagueness; it's naming a *how*. An outcome must survive **swapping the implementation underneath it**: "ingest X metrics within *t*" stays true whether you use startup configuration, Telegraf, or anything else, so it's an outcome. "startup configuration scripts are modular" names a mechanism - swap startup configuration out and the clause is meaningless - so it's an *implementation detail*, not an outcome (and not a DoD invariant either; see [the distinction among DoD, acceptance, and outcome](#three-things-people-confuse-dod-vs-acceptance-vs-outcome)). Note the test is **not** "is it specific?" - "zero manual post-build steps" is brutally specific and names no *how*, so it passes. **Ask: does this clause name a mechanism? If yes, it doesn't belong in the outcome.**

### The ops outcome catalog

Much of this kind of work delivers value as **reduced risk or reduced toil**, which people find genuinely hard to phrase as an outcome ("how do I put 'we made it less likely to break' into a done-condition?"). Use these four patterns - many operational epics are one of them. **Phrase them additively, not subtractively**: leadership hears "capacity reclaimed," not "work avoided."
...
| **Risk retirement** | A failure mode made impossible, or a single point of failure removed | "No host can register against the wrong regional endpoint; enforced by per-region secret and access-policy scoping" |
| **Capability at zero marginal cost** | New capability that scales **without added headcount** | "New regions' hosts stand up with no bespoke config - output grows, team size flat; proven in 2 regions" |
...
**❌ As written:** *"Epic to track work related to enabling and building a base image."* - an activity, no end state.

**✅ Outcome statement:**
> *When this is done, a single base image provisions Linux hosts across two runtime types with **zero manual post-build steps**, verified by **>=20 consecutive automated builds passing post-build validation**, and the **legacy build path is decommissioned** by end of Q3.*

That combines toil-reduction + reliability + risk-retirement, and every clause is checkable.

---

## Acceptance criteria (for stories / work packages)
...
> **Given** an instance launches in region-X, **when** startup configuration runs on first boot, **then** it registers against the region-X endpoint using the region-X registration token, and the registration status check succeeds.

**Checklist** - best for a set of concrete, independent conditions:
> - [ ] Script determines region/region from instance metadata with no hard-coded values
> - [ ] Correct registration endpoint URL + registration token fetched from the per-region secret
...
> ❌ *"PR submitted with startup configuration script that can dynamically determine registration endpoint URL and registration token."*

This is done the moment a PR exists - even if the script never successfully registers a single instance. The real done-condition is the ✅ Given/When/Then above: *an instance actually boots in a region and registers against the correct regional registration endpoint.* A PR is how the work arrives; it is not proof the work worked.
...
## Three things people confuse: DoD vs. Acceptance vs. Outcome
...
- **Drop the items that were never DoD material.** Two of the five don't belong in *any* of these three fields:
  - *"Private-environment-only configs leak into the shared base image"* - incoherent for this epic. A **golden/base image contains only what every system needs**; environment divergence belongs in image **layering** or runtime metadata resolution, never as baked-in conditionals. This isn't a mis-slotted bar, it's a misunderstanding of how images compose.
  - *"startup configuration scripts are modular"* - an **architecture decision**, and (per the ownership model in [the work hierarchy](#the-work-hierarchy)) the DoD is the requester's bar, not the implementers'; it has no standing to mandate a design. Architecture quality is real and worth pursuing - it just lives in **design / code review**, not a DoD (see the note at the end of this section). *(Note "modular **and managed in code**" was also redundant: "managed in code" = version-controlled = the item we already kept.)*
- **Add** the missing **Outcome statement** from [writing outcomes](#writing-outcomes-for-epics).

Two different fields. We were writing one, calling it the other, and smuggling two non-bars into the mix.

![Outcome, acceptance criteria, and the team Definition of Done close different claims at different levels; finishing child work does not itself prove the epic outcome.](assets/images/writing-work-items/three-tests-of-done.svg){#fig-three-tests-of-done}
...
## Demand-driven work is first-class

Many operations teams handle a large share of work that is **not roadmap work** - hardware failures, resource exhaustion, incidents, and a steady stream of standard requests, such as server builds or package signing. A planning standard that pretends everything is deliberate project work is useless to them. So this model names that work explicitly and gives it a place in the same reporting system.

Throughout this section, distinguish two things people lump together:

- **Roadmap-driven work** - improvement we *chose* to do. This is where the outcome, acceptance, and workmanship machinery above applies.
...
### The reporting-boundary principle
...
> **Where this lives:** requests are usually demand work types, governed this way - standard service-request handling applied where the model helps, without requiring full ITIL ceremony.
...
|---|---|---|---|
| **Roadmap / bespoke** | A roadmap improvement epic | **Authored per item** (outcome -> AC -> DoD) | Full outcome, acceptance, and workmanship machinery |
| **Demand / reactive** | A demand incident | By **resolution + structured capture** (cause, time) | Light; its *value* is as the evidence base feeding roadmap prioritization |
| **Repeatable / request** | A server-build request | **Frozen into the work type** once | Authoring collapses to form-filling + field validation |

The portable claim - independent of queue names or tooling - is that **authoring effort should scale with uncertainty.** Bespoke work earns the full machinery; repeatable work amortizes it into a form; reactive work is governed just enough to stay countable.
...
Prefer slices that each deliver a working end-to-end sliver (*"register in one region, end to end"*) over horizontal layers (*"write all the access-policy,"* then *"write all the scripts,"* then *"test everything"*). Vertical slices each have a real acceptance criterion and can be called done independently. Horizontal layers can't be "done" until the last one lands.

### Separate the decision from the delivery

If you don't yet know *how*, the unknown is its own work. Pull it out as a **Discovery work package** described above that:
...
2. **Story** - "Instance auto-registers to the correct regional endpoint on first boot" (AC from [acceptance criteria](#acceptance-criteria-for-stories--work-packages)).
3. **Task(s)** - "Create per-region registration secret," "Grant instance role read access to the secret."
...
- [ ] **Standing gates** are satisfiable - security/test/observability bar met or an approved exception linked
- [ ] It's a **vertical slice** that fits in a sprint
...
- [ ] It has an **Outcome / Exit Condition** - checkable, with a measure and a horizon (one or more conditions)
- [ ] The **team DoD** (the standing per-task quality bar) is linked/restated and is distinct from that outcome - you inherit it, you don't author one
...
Once work is executable, the organization needs an operating system that can see it, control how much is active, distinguish planned work from interruption, and turn repeated demand into evidence that the system itself should change.

<!-- Preview assembly source: Writing Work Items - Epics, Stories, and Tasks.md: sections 1–8; templates moved to Appendix A -->


\newpage

# Part IV: Operating, Improving, and Retiring the System {.part .unnumbered}
...
# Future Edition: Operating Cadence {.unnumbered}
...
> **Preview note — future edition**
>
> A future version will define a minimum viable cadence by decision rather than by inherited meeting name. It will state what must be reviewed daily, weekly, monthly, quarterly, and annually; which role or body may make each decision; which artifact enters; which state change must leave; and which urgent risks may bypass normal timing.
>
...
## The current design rules {.unnumbered .unlisted}
...
Cadence keeps active systems under review. Lifecycle discipline answers a different question: what changes when the organization has decided that a system should no longer receive normal investment at all?

<!-- Preview assembly source: Gap-map synthesis from Process Improvement, Work Intake, Managed Runoff, TDA, and RFP source material -->
...
> **Preview note — future edition**
>
> A future version will add a lightweight decision record, examples beyond deprecated services, clearer exception authorities by risk type, and the connection between runoff decisions and broader architecture or portfolio records.
...
### Freeze the Improvement Surface
...
The operating system now covers demand, commitment, execution, improvement, and retirement. The same loop applies to capability: organizations must decide what to develop internally, what to acquire externally, which evidence supports those judgments, and how the result changes the next decision.

<!-- Preview assembly source: Managed-Runoff-for-Deprecated-Services.md, complete argument with open TODO list converted to future-edition note -->


\newpage

# Part V: Developing and Acquiring Capability {.part .unnumbered}
...
### TDA Performance Evidence Ledger
...
![Development forms a closed loop in which aspiration becomes a target capability, real work, coaching, evidence, reconciliation, and a next decision that begins another cycle.](assets/images/talent-development/development-loop.svg){#fig-tda-development-loop}

---

The architecture identifies the loop and its records. Managers still need a recurring practice for producing evidence without turning feedback into character judgment or annual memory reconstruction.

<!-- Preview assembly source: Talent-Development-Architecture.md: Purpose, Core Model, boundaries, and implementation packet -->
...
# Future Edition: Hiring Capability {.unnumbered}
...
## Hiring and Free Agency {.unnumbered .unlisted}
...
Hiring acquires capability through a person. Vendor selection acquires capability through a commercial and technical relationship. The ethical, legal, and developmental obligations differ, but both decisions expose how easily an organization can confuse a persuasive claim with proof.

<!-- Preview assembly source: Talent-Development-Architecture.md: Hiring and Free Agency; future scope from 0.Organizational-Excellence-Book-Gaps.md -->
...
# Part VI: Reconciling the Enterprise {.part .unnumbered}
...
# Future Edition: Reconciling the Enterprise {.unnumbered}
...
> **Preview note — future edition**
>
> A future version will define benefits realization across projects and portfolios, the executive translation layer, evidence-retention and data-quality rules, and the boundary between local learning records and upward reporting.
>
...
## The current enterprise rule {.unnumbered .unlisted}
...
The enterprise evidence problem becomes sharper when AI can produce, summarize, and recombine organizational artifacts at very low cost. Cheap documentation can close loops, or it can make loop theater cheaper.

<!-- Preview assembly source: 0.Organizational-Excellence-Book-Gaps.md synthesis from Work Intake, RFPs, Managed Runoff, TDA, and Process Improvement -->


\newpage

# Future Edition: AI, Context, and Organizational Leverage {.unnumbered}
...
> **Preview note — future edition**
>
> A future version will develop the context-control-plane argument: how an organization reconstructs the right context for a decision, aligns tools with its cognitive capital, preserves authority boundaries, and prevents generated synthesis from replacing accountable judgment.
>
...
## Technology Cannot Substitute for Development Architecture {.unnumbered .unlisted}
...
## The operating test {.unnumbered .unlisted}
...
A book about operating systems should end by showing how to begin without installing the whole system at once. Adoption has its own capacity cost, evidence requirements, and failure modes.

<!-- Preview assembly source: Talent-Development-Architecture.md: Technology Cannot Substitute for Development Architecture; future scope from gap map -->
...
Use this as a working worksheet before a difficult, high-stakes, or emotionally loaded conversation.

## What are the relevant facts, feelings and context for this conversation?
...
## What may be important to the other person in this conversation?
...
## What are the possible areas of alignment / misalignment for this conversation?
...
## How am I going to open this conversation?  
### Opening statements

5.

<!-- Preview assembly source: Conversation Planner.md, complete worksheet -->
...
### Personal And Role Context

| Field | Response |
|---|---|
| Employee |  |
| Current role |  |
| Role family or discipline |  |
| Team or work group |  |
| Manager |  |
| Date in current role |  |
| Current level or operating mode |  |
| Date plan agreed |  |
| Review cadence |  |

### Current Role And Scope

What is the person currently accountable for?

| Accountability | Expected Outcome | Current Evidence | Notes |
|---|---|---|---|
|  |  |  |  |
|  |  |  |  |
|  |  |  |  |

What work has the person actually been doing that is not visible in the formal role description?

| Hidden Or Informal Work | Why It Exists | Impact On Capacity | Keep, Stop, Or Formalize? |
|---|---|---|---|
|  |  |  |  |
|  |  |  |  |
...
Roles, domains, systems, or operating modes the person wants to explore:

| Target | Why It Matters | Time Horizon | Business Need Or Useful Work |
|---|---|---|---|
|  |  |  |  |
|  |  |  |  |

### Preferences, Constraints, And Energy
...
|---|---|---|
| Work that creates energy |  |  |
| Work that drains energy |  |  |
| Work the person seeks out |  |  |
| Work the person avoids despite competence |  |  |
| Constraints affecting output |  |  |
| Conditions that help the person learn |  |  |
| Conditions that make learning harder |  |  |

### Current Capability Snapshot

| Capability, Skill, Domain, Or Behavior | Evidence Already Demonstrated | Confidence Level | Notes |
|---|---|---|---|
|  |  |  |  |
|  |  |  |  |
|  |  |  |  |

Completed education, training, certifications, labs, or self-study:

| Learning Completed | Why It Matters | Has It Been Applied In Real Work? | Evidence |
|---|---|---|---|
|  |  |  |  |
|  |  |  |  |
...
Development needs should be stated as observable gaps, not personality traits.

| Development Need | Current Evidence | Target Evidence | Why It Matters |
|---|---|---|---|
|  |  |  |  |
|  |  |  |  |
|  |  |  |  |
...
Choose a small number of targets. A plan with fifteen goals is usually a wish list, not an operating plan.

| Target | Transition Gate | Safe-Rep Opportunity | Manager Support Needed | Evidence Of Progress |
|---|---|---|---|---|
|  |  |  |  |  |
|  |  |  |  |  |
|  |  |  |  |  |
...
What does the manager agree with?

What does the manager see differently?

What organizational context does the employee need?

What opportunities can the manager realistically create or negotiate?

What support will the manager provide?

What support is not currently available?

### Next Decisions

- [ ] Move one or more targets into the TDA 70-20-10 Development Plan.
- [ ] Complete or refresh the TDA Competency Calibration.
- [ ] Create a stretch proposal.
...
This template turns competency language into evidence. It is not a personality assessment and it is not a performance rating. It is a shared calibration tool for answering one question:

What can this person currently demonstrate, what does the next operating mode require, and what safe reps would close the gap?

### Use

1. Select the current role, target role, target scope, or next operating mode.
2. Identify the competencies that actually matter for that target.
...
|---|---|---|
| Awareness | Understands the concept and can recognize it in context. | Can explain the idea, identify examples, and ask useful questions. |
| Guided execution | Can perform the work with support, review, or close supervision. | Completes bounded tasks when expectations, examples, and review are available. |
| Independent execution | Can perform the work reliably inside normal scope. | Delivers the work without unusual support and handles routine tradeoffs. |
| Broader coordination | Can coordinate the work across people, systems, or teams. | Anticipates dependencies, negotiates tradeoffs, and helps others execute. |
| System ownership | Can improve the operating system around the work. | Changes standards, processes, tooling, or strategy in ways that create leverage beyond direct execution. |
...
| Manager |  |
| Current role |  |
| Target role, scope, or operating mode |  |
| Date started |  |
| Date reviewed |  |
| Calibration cadence |  |
...
| Competency | Observable Definition | Target Stage | Employee Self Stage | Employee Evidence | Manager Stage | Manager Evidence | Gap To Close | Safe-Rep Path |
|---|---|---|---|---|---|---|---|---|
|  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |  |  |
...
Where did the employee and manager agree?

Where did the employee rate themselves higher than the manager did?

Where did the manager rate the employee higher than the employee did?

What evidence changed either person's view?

What was based on observed behavior?

What was based on self report?

What was based on interpretation or assumption?

Which gaps matter for the current role?

Which gaps matter only for a future role?

### Development Target Selection

Select the few gaps that matter most.

| Selected Gap | Why This Gap Matters | Development Target | Evidence Required Before Increasing Scope |
|---|---|---|---|
|  |  |  |  |
|  |  |  |  |
|  |  |  |  |
...
What work, access, feedback, coaching, or review will the manager provide?

What risks must stay bounded?
...
This template translates development needs into work. The ratio is not accounting. It is a design principle:

- 70 percent: real work, stretch assignments, shadowing, projects, process improvement, ownership reps, and other experience-based learning.
- 20 percent: coaching, mentoring, peer feedback, communities of practice, reviews, and learning from people who already perform the capability well.
- 10 percent: formal training, courses, reading, labs, certifications, reference material, or structured study.

The important rule is this: training is not evidence of readiness. Training supports readiness. Evidence comes from observed behavior under realistic conditions.

### Use

1. Start from a Personal Development Plan or Competency Calibration.
2. Choose one to three development objectives.
3. Define real work reps first.
4. Add feedback sources.
5. Add formal support.
6. Define risk boundaries and evidence.
7. Review progress at least quarterly.
...
| Manager |  |
| Current role |  |
| Target role, domain, or operating mode |  |
| Date agreed |  |
| Review cadence |  |
| Next checkpoint |  |
...
Write objectives that are specific, measurable, attainable, relevant, and time bounded. Do not turn the plan into a vague aspiration list.

| Development Objective | Transition Gate | Why It Matters | Desired Evidence |
|---|---|---|---|
|  |  |  |  |
|  |  |  |  |
|  |  |  |  |
...
- complete a bounded stretch assignment
- shadow a higher-risk workflow, then own a read-only step
- own a reversible change under review
- join a project team with clear scope
- lead a small process improvement
- document a recurring problem, then improve the workflow
- teach another person a task to test understanding
- present a technical or operational finding to the team
- support another team for a defined time box
...
- incident review
- design review
- code, document, ticket, or runbook review
- community of practice discussion
- customer, stakeholder, or receiving-team feedback
- shadowing debrief

| Objective | Feedback Source | Feedback Moment | Question To Ask | Evidence To Capture |
|---|---|---|---|---|
|  |  | Before / During / After |  |  |
|  |  | Before / During / After |  |  |
|  |  | Before / During / After |  |  |

### 10 Percent: Formal Support

Formal learning should prepare the person for a rep or help them make sense of a rep they already attempted.
...
| Objective | Formal Support | Why This Support Matters | How It Will Be Applied | Completion Evidence |
|---|---|---|---|---|
|  |  |  |  |  |
|  |  |  |  |  |
|  |  |  |  |  |
...
Use these questions at each checkpoint.

What is different about the person's work now compared with the last review?

What evidence shows improvement?

What evidence shows the gap remains?

What barriers appeared?

What support was missing?

Did the assignment create the intended rep?

Was the risk boundary correct?

Does the plan need to change?

What is the next safe rep?

### Reconciliation

| Objective | Evidence Produced | Capability Built? | Next Decision |
|---|---|---|---|
...
This template prevents annual review from becoming memory reconstruction. It connects accountabilities, measures, changed conditions, development actions, employee reflection, manager observation, and next decisions into one evidence record.

The ledger is not a surveillance file. It is a reconciliation tool. It should preserve useful evidence without turning every developmental conversation into a disciplinary artifact.
...
| Label | Meaning |
|---|---|
| Observed behavior | Something directly seen in work, meetings, incidents, artifacts, delivery, or collaboration. |
| Outcome measure | A metric, target, service level, completion record, quality result, or other measured result. |
| Work artifact | A runbook, document, design, automation, ticket, test, project output, or other created object. |
| Self report | The employee's account of what happened, what they intended, or what they learned. |
| Manager interpretation | The manager's read on what the evidence means. |
| Peer or stakeholder input | Feedback from someone affected by, or close to, the work. |
| Assumption | A claim that still needs confirmation. |
...
| End-cycle review date |  |

### Accountabilities And Measures

| Accountability | Expected Measure Or Observation | Evidence Source | Notes |
|---|---|---|---|
|  |  |  |  |
|  |  |  |  |
|  |  |  |  |
...
| Date | Accountability Or Development Target | Evidence Label | What Happened | Impact | Follow-Up |
|---|---|---|---|---|---|
|  |  |  |  |  |  |
|  |  |  |  |  |  |
|  |  |  |  |  |  |
...
Use this section when the assignment changed, load changed, priorities conflicted, support disappeared, a hidden workstream appeared, or the system around the person changed.

| Change | Date | Effect On Work | Management Response |
|---|---|---|---|
|  |  |  |  |
|  |  |  |  |
...
| Development Target | Real Work Rep | Feedback Source | Evidence Produced | Next Safe Rep |
|---|---|---|---|---|
|  |  |  |  |  |
|  |  |  |  |  |
...
Evidence both agree on:

Evidence still unclear:

Support or scope changes needed:

### End-Cycle Reconciliation

What did the person deliver?

What capability became visible?
...
What should the next plan change?
...
<!-- Preview assembly source: TDA-Personal-Development-Plan-Template.md; TDA-Competency-Calibration-Template.md; TDA-70-20-10-Development-Plan-Template.md; TDA-Performance-Evidence-Ledger-Template.md -->
...
*An implementation guide for creating a recurring place where leadership thinking happens.*

## Purpose

The Socratic Leadership Seminar is a leadership reading group, not a class, a change initiative, or a disguised planning meeting.

Participants read credible leadership and management research, then meet periodically to explore what the material reveals, where it contradicts their experience, and which questions remain unresolved. The seminar does not attempt to force agreement or produce an action plan. It creates repeated opportunities for leaders to encounter serious ideas, test those ideas against experience, and update their mental models over time.

The fixing, if it happens, is a byproduct.

## Where It Fits in Talent Development Architecture

Talent Development Architecture treats growth as a combination of real work, learning from other people, and formal support. For leaders, the seminar is one mechanism for learning from others:

- **70 percent real work:** coaching people and carrying actual management responsibility
- **20 percent learning from others:** peer exploration, mentoring, observation, and the seminar
- **10 percent formal support:** reading, courses, case studies, and other structured study

The ratios are design guidance, not timekeeping requirements.

The seminar cannot replace real management practice or direct coaching. It gives leaders a place to examine the mental models they bring to that work.

## Cohorts

Begin with a voluntary group. A useful starting cohort may be current directors and senior leaders, prospective managers, or another leadership population appropriate to the organization.

Current and prospective leaders do not have to be placed in the same cohort. Established-leader seminars can draw heavily on management experience. Leadership-discovery cohorts can help people test whether facilitation, coaching, and the development of others are work they genuinely enjoy.

As participation grows, multiple groups can operate in parallel. Participants should be able to move among compatible groups rather than becoming permanently assigned to one circle.

## Source Selection

The material should come from credible sources: peer-reviewed research, university departments, established research organizations, serious management publications, or well-documented organizational case studies.

Source credibility does not require agreement. A polished article may overstate its evidence. A widely used framework may collapse under lived experience. Those tensions are discussion material.

The group should agree on:

- the meeting cadence
- a realistic reading load
- the standard for acceptable sources
- who selects each session's material
- how far in advance the material must be distributed

Two weeks is a useful minimum for distributing the reading. The reading load should be small enough that preparation remains realistic for the full group.

## Rotating Facilitation

One participant facilitates each session, with responsibility rotating through the group.

The facilitator selects and distributes the material, opens the discussion, notices connections, surfaces tensions, and protects the purpose of the meeting. Rotation serves two purposes: everyone has to stretch their leadership thinking far enough to choose material worth discussing, and everyone receives practice facilitating a room.

The facilitator is a co-explorer, not a guide leading the group toward a predetermined conclusion.

A useful opening sounds like:

> These articles challenged some assumptions I had about change communication. I am curious what everyone else made of them.

The wrong opening is a disguised argument:

- "I found research that proves my view."
- "Let us decide how to apply this."
- "Here is what everyone should take away."

## Session Shape

A 60-minute session is a useful default. Extend it only when group size and discussion quality justify the additional time.

### Open with genuine curiosity

Spend the first few minutes asking what struck people, what seemed true, what felt overstated, and what created disagreement.
...
- Does this match your experience or contradict it?

Do not organize the answers too early. Let the group's first reactions reveal where the energy is.

### Let the discussion find its energy

The facilitator should:

1. **Notice connections.** Bring earlier comments back when two participants appear to be describing related ideas.
2. **Surface tensions.** Make competing interpretations visible without rushing to resolve them.
3. **Invite lived experience.** Ask where participants have seen the idea play out without turning one person's experience into a referendum on the whole organization.
4. **Return to the research.** Introduce a provocative claim, method, or finding when the discussion loses energy.
5. **Let disagreement breathe.** Ask people to explain what they do not accept instead of defending the material.

Real workplace examples are welcome when they deepen the discussion. One person's current problem should not take over the session or turn the group into an ad hoc working meeting.

### Close with an open question

Do not end by asking what everyone will do differently.

Ask instead:

- What are you still thinking about?
- What question did we leave unresolved?
- Which idea became less clear after discussing it?
...
![A three-stage seminar session moves from credible material through collective inquiry to an open question, while the facilitator protects curiosity rather than forcing agreement or action.](assets/images/socratic-leadership-seminar/session-shape.svg){#fig-socratic-session-shape}
...
![ELMO keeps disagreement and real examples inside the seminar while routing solution design and implementation work elsewhere.](assets/images/socratic-leadership-seminar/elmo-boundary.svg){#fig-socratic-elmo-boundary}

## The Whiteboard

Use the whiteboard minimally. Its purpose is to hold threads of thought while the conversation unfolds.

Useful entries include:

- themes participants notice
- questions that surface
- tensions or contradictions
- brief examples from experience
- a parking lot for important topics that do not fit the session

The board should not drive toward decisions, owners, deadlines, or action items. At the end of the session, it is evidence of thinking, not a plan.

A photograph of the board may be shared with the next session's topic. No additional artifact is required.

## Facilitator Discipline

The hardest facilitation requirement is genuine curiosity. The facilitator must be willing for experienced leaders to reject the material, interpret it differently, or reach no conclusion at all.

The useful stance is not "these people need to learn this." It is "these people have experience the research may not capture; I want to understand what happens when the two meet."

Questions that preserve that stance include:

- What systemic pressure makes the recommended behavior difficult?
- What does lived experience reveal that the research does not?
- Why might a reasonable leader choose the apparently weaker approach?
- Which assumption does the disagreement expose?

## Measurement Boundary

The seminar is a deliberately unmeasured input. Attendance proves presence. A worksheet proves that a worksheet was completed. Neither proves that a leader's mental model changed.

That does not make the seminar unfalsifiable. Its effects, if any, should eventually appear downstream: better coaching behavior, more thoughtful allocation of developmental reps, stronger facilitation, useful references to earlier discussions, or participants voluntarily requesting the next reading.
...
![The seminar may influence mental models that leaders carry into real work, where downstream behavior can be observed without claiming that the seminar directly caused it.](assets/images/socratic-leadership-seminar/signals-not-scores.svg){#fig-socratic-signals-not-scores}

## The Long Game

The value compounds across sessions.

After several meetings, participants may begin connecting new material to earlier conversations. Later, they may bring their own research, test an idea in real work, or recognize a management pattern they previously lacked language for.

The seminar does not exist to fix leaders. It creates a recurring place where leadership thinking happens. That is enough work for one room.

<!-- Preview assembly source: Socratic-Leadership-Seminar.md, complete article -->
