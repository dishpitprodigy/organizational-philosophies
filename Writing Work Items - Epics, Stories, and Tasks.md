...
Throughout, we use two anonymized composite work items as worked examples - one epic and one task - because the lessons are easier to follow with realistic material. Their **problem-version** content lives in the [**Example tickets**](#the-example-tickets-problem-version) tab; the analysis below refers to them as the **Example Epic** and the **Example Task**:

- the **Example Epic** - a base-image epic
- the **Example Task** - a startup registration item (typed a Task; child of the epic above)

Both are useful precisely because they show what to fix.

---

## The example tickets (problem version)

> **Why these are here.** This standard uses two anonymized composite work items as worked examples - one Epic, one Task - because the lessons are easier to follow with realistic material. Their **problem-version** content is summarized here so the rest of the document is self-contained; Sections 1–9 refer to them as the **Example Epic** and the **Example Task**. They're useful precisely because they show what to fix - not to point at any real team or author.
...
> Epic to track work related to enabling and building a base image. This focuses on the base image itself and not necessarily automating provisioning with an automation pipeline or self-service flow, which will be covered in the Base Image Pipeline Epic.
>
> Note: Some configurations are only applicable in one environment. Other environments get network settings from their own platform services. As this image will eventually be used for provisioning hosts in multiple environments, there should be logic to avoid applying environment-specific configuration in the wrong place.

**Definition of Done for all work items in this Epic (problem version):**

- image-based configurations should be tracked in the image-build repo
- environment-specific configurations should be skipped outside the environment where they apply
- startup configuration scripts should be modular and managed in code
...
- all configurations should have a regression test or validation test in a final post-build validation step
  ...
  - these images will eventually be used across multiple runtime types, so runtime-specific logic should account for environment or machine type. We do not need to make every function work in all runtimes now, but the code should not be written in a way where significant refactor would be necessary to do so later.

*What's wrong with this - and the corrected version - is worked through in [Section 1](#1-why-this-exists), [Section 3](#3-writing-outcomes-for-epics), and [Section 5](#5-three-things-people-confuse-dod-vs-acceptance-vs-outcome); the corrected epic is in [Section 9.1](#91-epic-template).*
...
> This work item will identify a method for dynamically targeting the correct regional registration endpoint.
>
> I would propose we use a startup configuration script that reads instance metadata to determine the host's region. Then, based on OS major version, it will fetch a secret containing the registration endpoint URL and registration token.
>
> The secret will be created in: *[example secret store]*. The instance role will be updated with permission to read the secret.
>
> Please note we are not using the vendor registration command. We are pulling a script from the target regional registration endpoint:

```
startup:
  - fetch registration script from <regional registration endpoint>
  - run registration script
  - enable required repositories
  - disable platform-default repositories
```

> The methodology we use to dynamically get the registration endpoint URL and registration token should be transportable to other environments, so ideally we can share a single base image to other environments while only needing to worry about the secrets / key and access-policy permissions in the destination environments.

**Acceptance Criteria (problem version):**
...
*Why this is mis-leveled - and how to split it - is in [Section 2](#2-the-work-hierarchy), [Section 4](#4-acceptance-criteria-for-stories--work-packages), and [Section 7](#7-splitting--sizing); the extracted story is in [Section 9.2](#92-story--work-package-template).*
...
> **Who owns what (this matters for the rest of the doc):** an epic is, in effect, a **broad-stroke request from the business** - and its **Outcome** and **Definition of Done are owned by the requester / standard-owner, not by the implementers.** The people choosing *how* to build it don't get to rewrite the bar they're measured against. The practical consequence shows up in [Section 5](#5-three-things-people-confuse-dod-vs-acceptance-vs-outcome): because the DoD isn't the implementers' to write, it can only hold things the business can legitimately demand - **substitutable, checkable invariants** ("works on both runtimes," "complies with our standards") - never *architecture choices* like "make it modular," which would be the requester reaching into the implementers' domain.
...
top demand drivers, ranked by count × hours
   ...
   spawn roadmap epic to retire the driver
        │                                   │
        ▼                                   ▼
   reported as %                  outcome measured in demand data (Section 3)
...
|---|---|---|---|
| **Roadmap / bespoke** | A roadmap improvement epic | **Authored per item** (outcome -> AC -> DoD) | Full machinery, Sections 2–5 |
